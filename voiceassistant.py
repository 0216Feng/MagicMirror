# https://gitee.com/zibuyu2015831/xunfei-spark-api/tree/master
...
class SparkGPT(object):
    # 理论上，token与字数的换算关系为 1tokens 约等于1.5个中文汉字 或者 0.8个英文单词
    # 为了确保请求不出错，这里设定临界值，即：
    #   实际允许输入的中文字数=允许的最大token数*1.5*0.9
    #   实际允许输入的英文字数=允许的最大token数*0.8*0.9
    # 可根据实际需要自定义。
    input_token_percent = 0.9

    def __init__(self, prompt=None, language="general"):

        if not os.path.exists('config.json'):
            raise Exception("config.json文件不存在！请检查！")
        ...
        # 获取版本与tokens设定映射字典
        self.versions_dict = self.spark_info.get('version_dict', {})

        # 判断domain输入是否合法
        if self.domain not in self.versions_dict:
            raise Exception(f"domain 【{self.domain}】 not in version_dict")

        self.language = language  # 文本语言设置，用于计算token总限制数。
        if self.language == "chinese":  # 纯中文文本
            self.max_length = int(self.versions_dict[self.domain]['max_output_tokens'] * 1.5 * self.input_token_percent)
        elif self.language == "english":  # 纯英文文本
            # 假设英文单词平均长度为5
            self.max_length = int(
                self.versions_dict[self.domain]['max_output_tokens'] * 0.8 * self.input_token_percent) * 5
        else:  # 中英文混杂文本：换算比例取1.2，此值可根据实际需要进行修改
            self.max_length = int(self.versions_dict[self.domain]['max_output_tokens'] * 1.2 * self.input_token_percent)

        self.text = []  # 历史会话列表
        self.single_answer_data = None
        self.all_answers_data = []  # 所有答案的流式信息

        self.single_answer: str = ""  # 用于流式接收回答
        self.error_msg: str = ""  # 错误文本
        self.uid: str = ""
    ...
    def set_max_tokens(self, value: int = 2048) -> int:
        """
        设置max_tokens，不同版本允许的最大值不同
        :param value: int类型，允许输出的最大tokens数
        :return: max_tokens，成功时返回设定的值，失败时返回0
        """
        ...
        if not isinstance(value, int):
            self.logger.error(f"max_tokens重设失败，传入值--【{value}】--非整数字")
            return 0

        max_tokens = int(value)

        if 1 <= max_tokens <= self.versions_dict[self.domain]['max_input_tokens']:
    ...
    def set_top_k(self, value: int = 4) -> int:
        """
        设置top_k，决定输出结果的随机性，取值为[1，6],默认为4
        :param value:  int类型
        :return: top_k，成功时返回设定的值，失败时返回0
        """
    ...
    def set_temperature(self, value: float = 0.5):
        """
        设置temperature，决定输出结果的随机性，取值范围 (0，1] ，默认值0.5
        :param value:  float类型
        :return: temperature，成功时返回设定的值，失败时返回0
        """
    ...
    def set_language(self, language: Literal['chinese', 'english', 'general']):
        """
        设置通讯语言，不同语言允许的最大输入字数不同
        :param language: 通讯语种选择，主要用于输入tokens换算。
        :return:
        """
    ...
    def on_open(self, ws):
        """收到websocket连接建立的处理:新建立一个线程负责通讯"""
    ...
    def talk(self, is_history: bool = False, callback_func=None):
        """
        会话（连续提问）
        :param is_history: 是否记住之前的会话记录
    ...
    def ask(self, question, flow_print=False, is_history: bool = False, callback_func=None):
        """
        提问（一次性问答）
        :param question: 用户的提问文本
        :param flow_print: 是否流式打印结果
        :param is_history: 是否记住之前的会话记录
        :param callback_func: 回调函数
        :return: 返回AI回应
        """
